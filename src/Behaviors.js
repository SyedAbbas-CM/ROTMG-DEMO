...
/**
 * StayBack behavior - Move away from entities within range
 */
export class StayBack extends Behavior {
  /**
   * @param {number} speed - Speed multiplier
   * @param {number} distance - Distance to maintain
   * @param {string} entity - Specific entity type to avoid (null for any)
   */
  constructor(speed = 1.0, distance = 8, entity = null) {
    super();
    this.speed = speed;
  ...
  execute(index, enemyManager, bulletManager, target, deltaTime, stateData) {
    // Initialize cooldown state
    if (!stateData.stayBackCooldown) {
      stateData.stayBackCooldown = 1000;
    }
    ...
    // Find nearest entity to avoid
    const nearestEntity = this.findNearestEntity(index, enemyManager, target);
    
    if (nearestEntity) {
      // Calculate direction away from entity
      const dx = nearestEntity.x - enemyManager.x[index];
      const dy = nearestEntity.y - enemyManager.y[index];
      const distance = Math.sqrt(dx * dx + dy * dy);
      ...
      // Update cooldown
      if (stateData.stayBackCooldown <= 0) {
        stateData.stayBackCooldown = 1000;
      } else {
        stateData.stayBackCooldown -= deltaTime * 1000;
      }
    }
  }
  
  findNearestEntity(index, enemyManager, target) {
    // Simplified - in a real implementation you'd search through all entities
    if (target) {
      const dx = target.x - enemyManager.x[index];
      const dy = target.y - enemyManager.y[index];
      const distance = Math.sqrt(dx * dx + dy * dy);
...
/**
 * StayCloseToSpawn behavior - Return to spawn location when too far away
 */
export class StayCloseToSpawn extends Behavior {
  /**
   * @param {number} speed - Speed multiplier
   * @param {number} range - Maximum distance from spawn
   */
  constructor(speed = 1.0, range = 5) {
    super();
    this.speed = speed;
    this.range = range;
  }
  ...
  execute(index, enemyManager, bulletManager, target, deltaTime, stateData) {
    ...
    if (!stateData.spawnPosition) {
      stateData.spawnPosition = {
        x: enemyManager.x[index],
        y: enemyManager.y[index]
      };
    }
    ...
    // Skip for pets with owners (if applicable)
    if (enemyManager.isPet && enemyManager.isPet[index] && enemyManager.owner && enemyManager.owner[index]) {
      return;
    }
    
    const spawn = stateData.spawnPosition;
    const dx = spawn.x - enemyManager.x[index];
    const dy = spawn.y - enemyManager.y[index];
    const distanceFromSpawn = Math.sqrt(dx * dx + dy * dy);
    
    if (distanceFromSpawn > this.range) {
      // Move back toward spawn
      const dirX = dx / distanceFromSpawn;
      const dirY = dy / distanceFromSpawn;
...
export class ReturnToSpawn extends Behavior {
  ...
  execute(index, enemyManager, bulletManager, target, deltaTime, stateData) {
    ...
    if (distance > 1.0) { // Small threshold to avoid infinite micro-movements
      // Move toward spawn
      const dirX = dx / distance;
      const dirY = dy / distance;
...
/**
 * Spawn behavior - Spawn entities at current location
 */
export class Spawn extends Behavior {
  /**
   * @param {string} entityType - Type of entity to spawn
   * @param {number} maxChildren - Maximum number of spawned entities
   * @param {number} initialSpawn - Initial spawn percentage (0-1)
   * @param {Object} cooldown - Cooldown between spawns {min, max}
   * @param {boolean} givesNoXp - Whether spawned entities give XP
   */
  constructor(entityType = 'basic_enemy', maxChildren = 5, initialSpawn = 0.5, cooldown = {min: 2000, max: 4000}, givesNoXp = true) {
    super();
    this.entityType = entityType;
    this.maxChildren = maxChildren;
    this.initialSpawn = Math.floor(maxChildren * initialSpawn);
    this.cooldown = cooldown;
    this.givesNoXp = givesNoXp;
  }
  ...
  execute(index, enemyManager, bulletManager, target, deltaTime, stateData) {
    ...
    if (!stateData.spawnState) {
      stateData.spawnState = {
        currentNumber: 0,
        remainingTime: this.getRandomCooldown(),
        initialized: false
      };
    }
    
    const state = stateData.spawnState;
    
    // Initial spawn on first execution
    if (!state.initialized) {
      for (let i = 0; i < this.initialSpawn; i++) {
        this.spawnEntity(index, enemyManager);
        state.currentNumber++;
      }
    ...
    // Spawn new entity if cooldown elapsed and under max
    if (state.remainingTime <= 0 && state.currentNumber < this.maxChildren) {
      this.spawnEntity(index, enemyManager);
      state.remainingTime = this.getRandomCooldown();
      state.currentNumber++;
    }
  }
  
  spawnEntity(index, enemyManager) {
    // This would integrate with your enemy spawning system
    // For now, we'll add a flag that can be checked by the spawning system
    if (enemyManager.requestSpawn) {
      enemyManager.requestSpawn({
        type: this.entityType,
        x: enemyManager.x[index],
        y: enemyManager.y[index],
        worldId: enemyManager.worldId[index],
        givesNoXp: this.givesNoXp,
        parentId: enemyManager.id[index]
      });
    }
  }
  
  getRandomCooldown() {
    const range = this.cooldown.max - this.cooldown.min;
    return this.cooldown.min + Math.random() * range;
  }
}

/**
 * SpawnGroup behavior - Spawn groups of entities within radius
 */
export class SpawnGroup extends Behavior {
  /**
   * @param {string} group - Group name or array of entity types
   * @param {number} maxChildren - Maximum number of spawned entities
   * @param {number} initialSpawn - Initial spawn percentage (0-1)
   * @param {Object} cooldown - Cooldown between spawns
   * @param {number} radius - Spawn radius around spawner
   */
  constructor(group = ['basic_enemy'], maxChildren = 5, initialSpawn = 0.5, cooldown = {min: 2000, max: 4000}, radius = 2) {
    super();
    this.group = Array.isArray(group) ? group : [group];
    this.maxChildren = maxChildren;
    this.initialSpawn = Math.floor(maxChildren * initialSpawn);
    this.cooldown = cooldown;
    this.radius = radius;
  ...
  execute(index, enemyManager, bulletManager, target, deltaTime, stateData) {
    // Initialize spawn state
    if (!stateData.spawnGroupState) {
      stateData.spawnGroupState = {
        currentNumber: 0,
        remainingTime: this.getRandomCooldown(),
        initialized: false
      };
    ...
    const state = stateData.spawnGroupState;
    
    // Initial spawn on first execution
    if (!state.initialized) {
      for (let i = 0; i < this.initialSpawn; i++) {
        this.spawnEntityInRadius(index, enemyManager);
        state.currentNumber++;
      }
      state.initialized = true;
    }
  ...
  spawnEntityInRadius(index, enemyManager) {
    // Random position within radius
    const angle = Math.random() * Math.PI * 2;
    const distance = Math.random() * this.radius;
    const x = enemyManager.x[index] + Math.cos(angle) * distance;
    const y = enemyManager.y[index] + Math.sin(angle) * distance;
    
    // Random entity type from group
    const entityType = this.group[Math.floor(Math.random() * this.group.length)];
    
    // Check if position is passable (simplified)
    if (enemyManager.requestSpawn) {
      enemyManager.requestSpawn({
        type: entityType,
        x: x,
...
export class RelativeSpawn extends Behavior {
  /**
   * @param {string} entityType - Type of entity to spawn
   * @param {number} offsetX - X offset from spawner
   * @param {number} offsetY - Y offset from spawner
   * @param {Object} cooldown - Cooldown between spawns
   * @param {number} maxChildren - Maximum spawned entities
   */
  constructor(entityType = 'basic_enemy', offsetX = 0, offsetY = 0, cooldown = {min: 3000, max: 5000}, maxChildren = 3) {
    super();
    this.entityType = entityType;
  ...
  execute(index, enemyManager, bulletManager, target, deltaTime, stateData) {
    ...
    const state = stateData.relativeSpawnState;
    
    // Update spawn timer
    state.remainingTime -= deltaTime * 1000;
    
    // Spawn if cooldown elapsed and under max
    if (state.remainingTime <= 0 && state.currentNumber < this.maxChildren) {
      const spawnX = enemyManager.x[index] + this.offsetX;
      const spawnY = enemyManager.y[index] + this.offsetY;
